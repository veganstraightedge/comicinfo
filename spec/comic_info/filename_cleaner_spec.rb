...
RSpec.describe ComicInfo::FilenameCleaner do
  describe '#clean' do
    ...
    context 'with tags argument' do
      ...
      context 'when removing multiple tags with no spaces between' do
        let(:filename_tags) { %w[(Digital) (Zone-Empire) (c2c)] }
        let(:old_filename)  { 'Amazing Spider-Man #1 (2018) (Digital)(Zone-Empire)(c2c).cbr' }
        let(:new_filename)  { 'Amazing Spider-Man #1 (2018).cbr' }
    ...
    context 'with tags env var' do
      let(:cleaner) { described_class.new tags_env: environment_variable_name }

      context 'when loading tags from environment variable' do
        let(:old_filename) { 'Batman #1 (2021) (Digital) [Scan] {c2c}.cbz' }
        let(:new_filename) { 'Batman #1 (2021).cbz' }
      ...
      context 'when loading tags from non-existent environment variable' do
        let(:old_filename) { 'Batman #1 (2021) (Digital) [Scan] {c2c}.cbz' }
